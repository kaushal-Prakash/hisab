...
const getGroupExpenses = async (req, res) => {
  try {
    ...
    // Get member details
    const members = await User.find({ _id: { $in: group.members } }, 'name imageUrl');
      ...
      return {
        ...
        owes: Object.entries(ledger[memberId])
          ...
          .map(([to, amount]) => ({ 
            to, 
            amount,
            toName: memberDetails.find(m => m.id.toString() === to)?.name || ''
          })),
        owedBy: ids
          ...
          .map(other => ({ 
            ...
            fromName: memberDetails.find(m => m.id.toString() === other.toString())?.name || ''
          })),
      };
    });
